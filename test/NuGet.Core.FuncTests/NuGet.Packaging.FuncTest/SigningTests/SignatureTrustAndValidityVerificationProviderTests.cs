...
namespace NuGet.Packaging.FuncTest
{
    ...
    public class SignatureTrustAndValidityVerificationProviderTests
    {
        ...
        private static readonly SignedPackageVerifierSettings _repoSignatureVerificationSettings = new SignedPackageVerifierSettings(
            allowUnsigned: false,
            allowIllegal: false,
            allowUntrusted: false,
            allowIgnoreTimestamp: false,
        ...
        [CIOnlyFact]
        public async Task VerifyAsync_WithAuthorSignedPackage_ReturnsUnknown()
        {
            using (var test = await Test.CreateAuthorSignedPackageAsync(_testFixture.TrustedTestCertificate.Source.Cert))
            using (var packageReader = new PackageArchiveReader(test.PackageFile.FullName))
            {
                ...
                var result = await provider.GetTrustResultAsync(packageReader, primarySignature, _repoSignatureVerificationSettings, CancellationToken.None);
        ...
        public class RepositoryPrimarySignatures
        {
            ...
            public async Task VerifyAsync_WithValidSignature_ReturnsValid()
            {
                ...
                using (var packageReader = new PackageArchiveReader(test.PackageFile.FullName))
                {
                    ...
                    var status = await _provider.GetTrustResultAsync(packageReader, primarySignature, _repoSignatureVerificationSettings, CancellationToken.None);
            ...
            [CIOnlyFact]
            public async Task VerifyAsync_WithValidSignatureButNoTimestamp_ReturnsProhibited()
            {
                using (var test = await Test.CreateRepositoryPrimarySignedPackageAsync(
                    _fixture.TrustedRepositoryCertificate.Source.Cert))
                using (var packageReader = new PackageArchiveReader(test.PackageFile.FullName))
            ...
            public async Task VerifyAsync_WithUntrustedSignature_ReturnsUntrusted()
            {
                ...
                using (var packageReader = new PackageArchiveReader(test.PackageFile.FullName))
                {
                    ...
                    Assert.Equal(SignatureVerificationStatus.Untrusted, status.Trust);
                }
            }
            ...
            [CIOnlyFact]
            public async Task VerifyAsync_WithRevokedTimestampCertificate_ReturnsProhibited()
            {
                var testServer = await _fixture.GetSigningTestServerAsync();
                var certificateAuthority = await _fixture.GetDefaultTrustedCertificateAuthorityAsync();
            ...
            [CIOnlyFact]
            public async Task VerifyAsync_WithTamperedRepositoryPrimarySignedPackage_ReturnsValid()
            {
                var timestampService = await _fixture.GetDefaultTrustedTimestampServiceAsync();
        ...
        public class RepositoryCountersignatures
        {
            ...
            [CIOnlyFact]
            public async Task VerifyAsync_WithValidCountersignatureButNoTimestamp_ReturnsProhibited()
            {
                var timestampService = await _fixture.GetDefaultTrustedTimestampServiceAsync();
